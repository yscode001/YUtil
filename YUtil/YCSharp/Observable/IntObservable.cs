...
namespace YCSharp
{
    public class IntObservable
    {
        ...
        private event Action<int> Event_ValueChanged1;
        private event Action Event_ValueChanged2;

        public void AddListener1(Action<int> action, bool immediateTrigger = false)
        {
            if (action != null)
            {
        ...
        public void RemoveListener1(Action<int> action)
        {
            if (action != null)
            {
                Event_ValueChanged1 -= action;
        ...
        #region 私有化构造函数
        private IntObservable() { }
        private IntObservable(int initValue)
        {
            _value = initValue;
        }
        #endregion
        ...
        #region 静态构造函数
        public static IntObservable Create()
        {
            return new IntObservable();
        }
        public static IntObservable Create(int initValue)
        {
            return new IntObservable(initValue);
        }
        public static IntObservable Create(int initValue, Action<int> immediateTrigger)
        {
            IntObservable obs = new IntObservable(initValue);
        ...
        public static IntObservable Create(int initValue, Action immediateTrigger)
        {
            IntObservable obs = new IntObservable(initValue);
            if (immediateTrigger != null)
            {
        ...
        public static IntObservable Create(int initValue, Action<int> immediateTrigger1, Action immediateTrigger2)
        {
            IntObservable obs = new IntObservable(initValue);
